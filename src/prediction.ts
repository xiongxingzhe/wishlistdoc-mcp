import { SteamAppDetails, LaunchConePrediction } from './types.js';

export function runSalesConePrediction(
  app: SteamAppDetails,
  wishlists?: number,
  customPriceUSD?: number
): LaunchConePrediction {
  ...
  if (price > 25.0) priceMultiplier = 0.75;
  else if (price < 10.0 && price > 0) priceMultiplier = 1.2;

  const visualMultiplier = (app.movies_count || 0) >= 1 ? 1.05 : 1.0;

  const totalMultiplier = parseFloat(
    (langMultiplier * demoMultiplier * priceMultiplier * visualMultiplier).toFixed(3)
  );
  ...
  return {
    ...
    p10_pessimistic: {
      ...
      confidenceDescription:
        '90% probability above this floor. Pessimistic scenario (high launch day refund rate, negative review clusters, technical bugs).',
    ...
    p50_median: {
      ...
      confidenceDescription:
        '50% probability above this baseline. Median performance for peer indie games within the same genre.',
    ...
    p90_optimistic: {
      ...
      confidenceDescription:
        '10% probability of breakout. Viral tier (Overwhelmingly Positive >92%, persistent front-page algorithmic recirculation).',
      conversionRatePercent: parseFloat((effectiveConversionP50 * 2.5 * 100).toFixed(2)),
    },
    estimatedPriceUSD: price,
    year1Multiplier,
    empiricalMethodology:
      'Modeled using GameDiscoverCo empirical conversion medians (Simon Carless) and WishlistDoc pre-launch calibration algorithms with 270-day exponential decay.',
  };
}
