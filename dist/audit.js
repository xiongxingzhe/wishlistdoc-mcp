export function runStoreAudit(app) {
    // 1. Tag Accuracy (20%)
    ...
    const hasSimplifiedChinese = supportedLower.includes('simplified chinese') || supportedLower.includes('schinese');
    ...
    // Total Score & Grade
    const totalScore = Math.round(tagDim.weightedScore +
    ...
    if (demoDim.score === 0) {
        prescriptions.push({
            ...
            reason: 'A public demo qualifies your title for Steam Next Fest, the largest free organic conversion window on the platform. Titles with demos experience 3.4x higher daily wishlist retention.',
            executionSteps: [
    ...
    if (visualDim.score < 80) {
        prescriptions.push({
            ...
            reason: '70% of Steam shoppers evaluate only the first two screenshots during mouseover hover cards or store entry. Avoid concept art or splash screens in slots #1 and #2.',
            executionSteps: [
    ...
    if (descDim.score < 80) {
        prescriptions.push({
            ...
            reason: 'The short description is the only copy rendered on hover cards, search auto-completes, and mobile Steam feeds. Narrative lore should be secondary to genre micro-loops.',
            executionSteps: [
